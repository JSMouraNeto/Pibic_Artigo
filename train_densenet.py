...
# --- 3. ARQUITETURA DO MODELO (MODIFICADO PARA DENSENET-121) ---
class DenseNet121Classifier(nn.Module):
    """Classificador baseado na arquitetura DenseNet-121."""
    def __init__(self, num_classes, pretrained=True):
        super().__init__()
        # Carrega o modelo DenseNet-121 pré-treinado da torchvision
        self.backbone = models.densenet121(weights=models.DenseNet121_Weights.IMAGENET1K_V1 if pretrained else None)
        
        # Congela os parâmetros da parte de extração de características (features)
        for param in self.backbone.features.parameters():
            param.requires_grad = False
            
        # Extrai o número de características da camada classificadora original
        num_features = self.backbone.classifier.in_features # Para DenseNet-121, é 1024
        
        # Substitui o classificador original por uma nova camada linear
...
def train_and_evaluate(args, logger):
    ...
    # **MODIFICAÇÃO: Instancia o modelo DenseNet-121**
    model = DenseNet121Classifier(num_classes=len(args.classes)).to(device)
    
    # Otimiza apenas os parâmetros do novo classificador
    ...
    logger.info("--- Iniciando Treinamento com DenseNet-121 ---")
    for epoch in range(args.epochs):
        model.train()
        model.backbone.features.eval() # Mantém o backbone congelado em modo de avaliação
    ...
    # **MODIFICAÇÃO: A camada alvo é o último bloco denso (denseblock4) da DenseNet-121**
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Framework de Treinamento com DenseNet-121 e XAI")
    ...
    # **MODIFICAÇÃO: Tamanho de imagem padrão para DenseNet**
    parser.add_argument('--image_size', type=int, default=224, help="Tamanho da imagem (DenseNet usa 224x224)")
    # Dropout não é necessário na classe do modelo, pois a DenseNet não tem camadas de Dropout no classificador por padrão
    ...
    parser.add_argument('--batch_size', type=int, default=24) # DenseNet pode usar um pouco mais de memória
    ...
    if base_args.data_dir == 'caminho/para/seu/dataset' or not os.path.isdir(base_args.data_dir):
        ...
        # **MODIFICAÇÃO: Diretório de resultados específico para DenseNet-121**
        results_base_dir = "training_results_densenet121"
        ...
        run_args.model_save_path = str(output_dir / f"{dataset_name}_best_model_densenet121.pth")
