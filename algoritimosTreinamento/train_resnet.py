...
# --- 3. ARQUITETURA DO MODELO (MODIFICADO PARA RESNET-50) ---
class ResNet50Classifier(nn.Module):
    """Classificador baseado na arquitetura ResNet-50."""
    def __init__(self, num_classes, pretrained=True):
        super().__init__()
        # Carrega o modelo ResNet-50 pré-treinado da torchvision (usando os pesos V2, mais recentes)
        ...
        # Extrai o número de características da camada classificadora original ('fc')
        num_features = self.backbone.fc.in_features # Para ResNet-50, é 2048
...
def train_and_evaluate(args, logger):
    ...
    # **MODIFICAÇÃO: Instancia o modelo ResNet-50**
    model = ResNet50Classifier(num_classes=len(args.classes)).to(device)
    
    # Otimiza apenas os parâmetros do novo classificador ('fc')
    optimizer = optim.AdamW(model.backbone.fc.parameters(), lr=args.learning_rate, weight_decay=1e-4)
    criterion = nn.CrossEntropyLoss(weight=torch.FloatTensor(class_weights).to(device))
    scaler = torch.amp.GradScaler(device='cuda', enabled=torch.cuda.is_available())
    
    best_val_f1 = 0.0

    logger.info("--- Iniciando Treinamento com ResNet-50 ---")
    for epoch in range(args.epochs):
        model.train()
        # Coloca o backbone em modo de avaliação para desativar Dropout e normalizar estatísticas de BatchNorm
        model.backbone.eval()
        # Reativa o modo de treino apenas para a camada que estamos treinando
    ...
    # **MODIFICAÇÃO: A camada alvo é o último bloco convolucional (layer4) da ResNet-50**
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Framework de Treinamento com ResNet-50 e XAI")
    parser.add_argument('--data_dir', type=str, default='/home/mob4ai-001/Pibic_Artigo/data/bigger_base_clean', help='Diretório raiz contendo as pastas das classes.')
    parser.add_argument('--classes', nargs='+', default=['COVID', 'Lung_Opacity', 'Normal', 'pneumonia_bacterial','Viral Pneumonia'])
    parser.add_argument('--image_size', type=int, default=224, help="Tamanho da imagem (ResNet usa 224x224)")
    ...
    if base_args.data_dir == 'caminho/para/seu/dataset' or not os.path.isdir(base_args.data_dir):
        ...
        # **MODIFICAÇÃO: Diretório de resultados específico para ResNet-50**
        results_base_dir = "training_results_vfinal"
        ...
        run_args.model_save_path = str(output_dir / f"{dataset_name}_best_model_resnet50.pth")
