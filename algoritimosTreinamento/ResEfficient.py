...
class MedicalImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Retorna o caminho da imagem também, para o Grad-CAM
...
class AdvancedMedicalClassifier(nn.Module):
    ...
    def forward(self, x):
        return self.classifier(torch.cat([self.backbone1(x), self.backbone2(x)], dim=1))
...
class GradCAM:
    ...
    def __call__(self, x, class_idx=None):
        ...
        output.backward(gradient=one_hot, retain_graph=True) # retain_graph pode ser necessário
...
def train_and_evaluate(args, logger):
    ...
    logger.info("--- Iniciando Treinamento Padronizado ---")
    ...
    visualize_and_save_grad_cam(
        model=model,
        target_layer=model.backbone1.conv_head, # Camada final do EfficientNet
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_dir', type=str, default='/home/mob4ai-001/Pibic_Artigo/dataset_processado/', help='Diretório raiz contendo as pastas dos datasets.')
    parser.add_argument('--classes', nargs='+', default=['COVID', 'Normal'])     
    parser.add_argument('--image_size', type=int, default=256)
    parser.add_argument('--dropout', type=float, default=0.4)
    parser.add_argument('--epochs', type=int, default=25, help='Número máximo de épocas.')
    parser.add_argument('--learning_rate', type=float, default=3e-4, help='Taxa de aprendizado fixa.')
    ...
    results_base_dir = "ResEfficient_2"
    ...
    for dataset_path in [d for d in root_data_dir.iterdir() if d.is_dir()]:
        ...
        # Define os caminhos de saída para esta execução específica
