...
import timm # Adicionado para carregar o modelo EfficientNet-B4
import gc
import copy
...
def get_medical_transforms(image_size=380, is_training=True):
...
# --- 3. ARQUITETURA DO MODELO (MODIFICADO PARA EFFICIENTNET-B4) ---
class EfficientNetB4Classifier(nn.Module):
    """Classificador baseado na arquitetura EfficientNet-B4."""
    def __init__(self, num_classes, pretrained=True, dropout=0.5):
        super().__init__()
        # Carrega o modelo EfficientNet-B4 pré-treinado (sem o classificador final)
        ...
        # Número de características na saída do backbone (para B4, é 1792)
...
def train_and_evaluate(args, logger):
    ...
    # **MODIFICAÇÃO: Instancia o modelo EfficientNet-B4**
    model = EfficientNetB4Classifier(num_classes=len(args.classes), dropout=args.dropout).to(device)
    ...
    logger.info("--- Iniciando Treinamento com EfficientNet-B4 ---")
    for epoch in range(args.epochs):
        model.train()
        model.backbone.eval() # Mantém o backbone congelado em modo de avaliação
    ...
    # **MODIFICAÇÃO: A camada alvo é a ativação final (act2) da EfficientNet-B4**
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Framework de Treinamento com EfficientNet-B4 e XAI")
    ...
    # **MODIFICAÇÃO: Tamanho de imagem ideal para EfficientNet-B4**
    parser.add_argument('--image_size', type=int, default=380, help="Tamanho da imagem (EfficientNet-B4 usa 380x380)")
    ...
    # **MODIFICAÇÃO: Batch size menor devido ao tamanho da imagem**
    parser.add_argument('--batch_size', type=int, default=12, help='Batch size reduzido para imagens 380x380.')
    ...
    if base_args.data_dir == 'caminho/para/seu/dataset' or not os.path.isdir(base_args.data_dir):
        ...
        # **MODIFICAÇÃO: Diretório de resultados específico para EfficientNet-B4**
        results_base_dir = "training_results_efficientnet_b4"
        ...
        run_args.model_save_path = str(output_dir / f"{dataset_name}_best_model_efficientnet_b4.pth")
